...
public class WandItem extends Item implements MenuProvider {
    ...
    public void inventoryTick(ItemStack wand, Level level, Entity entity, int slotId, boolean isSelected) {
        ...
        if(cooldown >= 0) wand.set(ModDataComponents.STACK_COOLDOWN, --cooldown);
    ...
    public boolean isBarVisible(ItemStack wand) {
        return wand.has(ModDataComponents.WAND_PROPERTIES);
    }

    @Override
    public int getBarColor(ItemStack wand) {
        return wand.has(ModDataComponents.WAND_PROPERTIES)? 0x00def2: 0x000000;
    }

    @Override
    public int getBarWidth(ItemStack wand) {
        if(!wand.has(ModDataComponents.WAND_PROPERTIES)) {
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand usedHand) {
        ...
        if(wand.get(ModDataComponents.STACK_COOLDOWN) > 0) {
            return InteractionResultHolder.fail(wand);
        }

        List<SpellHolder> spells = ListUtil.mutable(wand.get(ModDataComponents.SPELL_HOLDER));
        ...
        wand.set(ModDataComponents.STACK_COOLDOWN, wandProperties.cooldown());
        wand.set(ModDataComponents.SPELL_HOLDER, spells);
        wand.set(ModDataComponents.WAND_PROPERTIES, wandProperties);
    ...
    public void appendHoverText(ItemStack wand, TooltipContext context, List<Component> tooltipComponents, TooltipFlag tooltipFlag) {
        if(!wand.has(ModDataComponents.SPELL_HOLDER)) {
            return;
        }
        ...
        List<SpellHolder> spells = wand.get(ModDataComponents.SPELL_HOLDER.get());
    ...
    public Optional<TooltipComponent> getTooltipImage(ItemStack wand) {
        return Optional.ofNullable(wand.get(ModDataComponents.WAND_PROPERTIES));
    }
    ...
    public static boolean hasNecessaryComponent(ItemStack wand) {
        return wand.has(ModDataComponents.SPELL_HOLDER) && wand.has(ModDataComponents.WAND_PROPERTIES) && wand.has(ModDataComponents.STACK_COOLDOWN);
    }
}
